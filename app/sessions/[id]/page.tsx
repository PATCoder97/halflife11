  ...
  return (
    <div className="space-y-9">
      <div>
        ...
        <p className="mt-2 text-concrete">{session.matches.length} trận đã ghi nhận</p>
      </div>
          ...
          {session.matches.map((match) => {
            ...
            return (
              <Card key={match.id} className="shadow-none">
                <p className="mb-4 text-[10px] font-black uppercase tracking-[0.2em] text-concrete">Encounter // {String(match.sequence).padStart(2, "0")}</p>
                <div className={`border-l-2 p-4 ${teamA[0]?.result === "WIN" ? "border-leaf bg-leaf/15 text-leaf" : "border-cream/20 bg-black/30 text-concrete"}`}>
                  <span className="font-black">{teamA.map((item) => `${item.player.name}${item.weapon ? ` [${item.weapon.name}]` : ""}`).join(" + ")}</span>
                ...
                <div className={`border-l-2 p-4 ${teamB[0]?.result === "WIN" ? "border-leaf bg-leaf/15 text-leaf" : "border-cream/20 bg-black/30 text-concrete"}`}>
                  <span className="font-black">{teamB.map((item) => `${item.player.name}${item.weapon ? ` [${item.weapon.name}]` : ""}`).join(" + ")}</span>
